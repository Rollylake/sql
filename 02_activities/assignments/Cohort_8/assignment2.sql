...
DROP VIEW IF EXISTS list_for_products;
CREATE VIEW IF NOT EXISTS list_for_products AS

SELECT 
coalesce (product_name, '') || ', ' || coalesce (product_size, '') || ' (' || coalesce (product_qty_type, 'unit') || ')' AS list_of_products
FROM product;

SELECT * from list_for_products ;
...
SELECT
    ...
    market_date,
    transaction_time,
    ROW_NUMBER() OVER (PARTITION BY customer_id  ORDER BY market_date, transaction_time) AS No_of_visit
...
FROM (
    SELECT 
        ...
        market_date,
        transaction_time,
        DENSE_RANK() OVER (PARTITION BY customer_id ORDER BY market_date DESC) AS No_of_visit
    FROM customer_purchases
) AS ranked_visits
WHERE No_of_visit= 1
